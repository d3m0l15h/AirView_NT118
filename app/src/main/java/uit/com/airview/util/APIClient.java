...
public class APIClient {
    ...
    private static Retrofit openWeatherMapRetrofit = null;
...
//            });
    ...
    public static Retrofit getOpenWeatherMapClient(Context context) {
        if(openWeatherMapRetrofit == null) {
            OkHttpClient client = getUnsafeOkHttpClient(context);

            openWeatherMapRetrofit = new Retrofit.Builder()
                    .baseUrl("https://api.openweathermap.org/")
                    .addConverterFactory(GsonConverterFactory.create())
                    .client(client)
                    .build();
        }
        return openWeatherMapRetrofit;
    }
}
